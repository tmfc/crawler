...
while(! feof($file))
{
	...
	try{
		...
		//die;
		//echo count($columns);
		if(!in_array(count($columns), array(23)))
		{
		...
		$info->owner = $columns[11];
		$info->owner_id = $columns[9];
		$info->enroll_time = $columns[16];
		$info->telphone = $columns[14];
		$info->mobile = $columns[15];
		$info->insurance_expire_date = $columns[16];
